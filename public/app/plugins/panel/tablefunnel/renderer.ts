...
export class TableRenderer {
  ...
  setColorState(value, style) {
    ...
    this.colorState[style.fillMode] = this.getColorForValue(numericValue, style);
  }
  ...
  renderCell(columnIndex, rowIndex, value, addWidthHack = false, maxColVal = -1) {
    ...
    let cellClass = '';
    let funnelTag = '';
    if (column.style.fillMode === 'percentage') {
      funnelTag =
        '<div class="progress-bar" style="width:' +
        Math.round(initialVal * 10000) / 100 +
        '%;opacity:' +
        column.style.fillOpacity +
        ';background-color:' +
        column.style.color +
        ';"></div>';
      this.colorState.cell = null;
    } else if (column.style.fillMode === 'value') {
      console.log('value gives: ' + Math.round(initialVal / maxColVal * 10000) / 100);
      funnelTag =
        '<div class="progress-bar" style="width:' +
        Math.round(initialVal / maxColVal * 10000) / 100 +
        '%;opacity:' +
        column.style.fillOpacity +
        ';background-color:' +
        column.style.color +
    ...
    columnHtml = '<td' + cellClass + style + '"' + '>' + funnelTag + columnHtml + '</td>';
    return columnHtml;
  }
  ...
  render(page) {
    ...
    //check each column for a max value Funnel and find the max value for each column that needs it
    const colMaxValues = [];
    for (let i = 0; i < this.table.columns.length; i++) {
      let maxVal = -1;
      console.log(this.table.columns[i].style.fillMode);
      if (this.table.columns[i].style.fillMode === 'value') {
        for (let j = 0; j < this.table.rows.length; j++) {
          if (this.table.rows[j][i] > maxVal) {
            maxVal = this.table.rows[j][i];
          }
        }
    ...
    console.log(colMaxValues);

    for (let y = startPos; y < endPos; y++) {
      const row = this.table.rows[y];
      let cellHtml = '';
      for (let i = 0; i < this.table.columns.length; i++) {
        cellHtml += this.renderCell(i, y, row[i], y === startPos, colMaxValues[i]);
      }
